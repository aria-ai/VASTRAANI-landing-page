...
const Footer = () => {
  ...
  return (
    <>
              ...
              <div key={index}>
                ...
                <ul className="space-y-2">
                  {section.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      {link.name === "Shipping Info" ? (
                        <button
                          onClick={() => openPolicy("shipping")}
                          className="text-cream-200 hover:text-gold-400 transition-colors duration-300 text-left"
                        >
                          {link.name}
                        </button>
                      ) : link.name === "Returns" ? (
      ...
      />
      <PolicyModal
        isOpen={activePolicy === "returns"}
        onClose={closePolicy}
        title={returnsPolicy.title}
        content={returnsPolicy.content}
      />
    </>
  );
};
