 ...
 */
const ManualCarousel = ({ images, alt, initialIndex = 0, onImageChange }) => {
...
const CollectionModal = ({
  ...
  const [currentImageIdx, setCurrentImageIdx] = useState(initialImageIndex);
  ...
  const handleDownload = () => {
    ...
    // Serve the hi-res JPG (originals kept alongside WebP)
    const jpgUrl = images[currentImageIdx].replace(".webp", ".jpg");
    const filename = `vastraani-${collection.id}-${currentImageIdx + 1}.jpg`;
        ...
        >
          ...
          >
            ...
            <div className="relative h-full flex flex-col md:flex-row overflow-hidden">
              ...
              >
                <ManualCarousel
                  key={`${collection.id}-${initialImageIndex}`}
                  ...
                  onImageChange={setCurrentImageIdx}
                />
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-burgundy-900/95 to-transparent p-6 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      className={`
                        ...
                        ${
                          showInfo
                            ...
                            : "bg-cream-50/20 text-cream-50 hover:bg-cream-50 hover:text-burgundy-900"
                        }
                      `}
                    ...
                    {/* Download button — subtle, hi-res JPG */}
                    <motion.button
                      onClick={handleDownload}
                      className="w-10 h-10 flex items-center justify-center rounded-full bg-cream-50/20 text-cream-50 hover:bg-cream-50 hover:text-burgundy-900 transition-colors"
                      whileHover={{ scale: 1.1 }}
                      whileTap={{ scale: 0.95 }}
                      aria-label="Download hi-res image"
                      title="Download hi-res photo"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    ...
                    <motion.button
                      ...
                      className="px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white font-serif font-semibold rounded-full flex items-center gap-2 transition-colors text-sm"
